...
#' 1. sorts the H2OFrame by columns sorted in by columns specified in groupbyCols and sortCols in the directions
#'   specified by the ascending for the sortCols.  The sort directions for the groupbyCols are ascending only.
...
#' 4. If sortColsSorted is TRUE, a final sort on the frame will be performed frame according to the sortCols and
#'    the sort directions in ascending.  If sortColsSorted is FALSE (by default), the frame from step 3 will be
#'    returned as is with no extra sort.  This may provide a small speedup if desired.
#'
#'
#' @param x The H2OFrame input to be sorted.
#' @param groupbyCols a list of column names or indices to form the groupby groups
#' @param sortCols a list of column names or indices for sorting
#' @param ascending a list of Boolean to determine if ascending sort (set to TRUE) is needed for each column in
#'  sortCols (optional).  Default is ascending sort for all.  To perform descending sort, set value to FALSE
#' @param newColName new column name for the newly added rank column if specified (optional).  Default name is
#'  New_Rank_column.
#' @param sortColsSorted Boolean to determine if the final returned frame is to be sorted according to the sortCols
...
#'
#' If the following commands are issued:
#' rankedF1 <- h2o.rank_within_group_by(train, c("Group_by_column"), c("Column_to_arrange_by"), c(TRUE), sortColsSorted=TRUE)
#' h2o.summary(rankedF1)
#'
...
#'
#' @export
h2o.rank_within_group_by <- function(x, groupbyCols, sortCols, ascending=NULL, newColName="New_Rank_column", sortColsSorted=FALSE) {
  ...
  .newExpr("rank_within_groupby", x, group.cols-1L, sort.cols-1L, sortdir, newColName, finalSortedOrder)
}

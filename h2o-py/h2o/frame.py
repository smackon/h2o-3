...
class H2OFrame(object):
    ...
    def rank_within_group_by(self, groupbyCols, sortCols, ascending=[], newColName="New_Rank_column", sortColsSorted=False):
        """
        ...
        :param sortColsSorted: Optional Boolean to denote if the returned frame should be sorted accoring to sortCols
        ...
        :return: a new Frame sorted by columns in sortCols with new rank column within the grouping specified
         ...
         If the following commands are issued:
         rankedF1 <- h2o.rank_within_group_by(train, ["Group_by_column"], ["Column_to_arrange_by"], [TRUE], sortColsSorted=True)
        ...
        if (sortColsSorted):
            finalSortedOrder=1
        return H2OFrame._expr(expr=ExprNode("rank_within_groupby",self,groupbyCols,sortCols,ascendingI,newColName, finalSortedOrder))
